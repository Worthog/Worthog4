...
// the following is too complicated lets use a simple version of getevents and convert it to getdevices
// take a look at the events page. 

export const getUserDevices = (userUid, activeTab) => async (dispatch, getState) => {
  dispatch(asyncActionStart());
  const firestore = firebase.firestore();
  const today = new Date(Date.now());
  let eventsRef = firestore.collection('event_attendee');
